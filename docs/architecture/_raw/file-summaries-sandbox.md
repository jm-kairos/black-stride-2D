...
- **Notable non-obvious dependencies:**
  ...
  - **~25 global keybindings are hardcoded inline** in `game_update` as `input_is_key_down(K) && !input_was_key_down(K)` edge tests — SHIFT, R, V, T, P, L, N, I, H, O, K, G, and F3–F12 (TAB retired with the pilot/auto-pilot toggle). There is no key-binding table, no remapping, and the mapping exists nowhere but these call sites.
