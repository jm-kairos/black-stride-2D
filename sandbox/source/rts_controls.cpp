...
static constexpr f32 FREE_CAM_ACCEL            = 2400.0f;  // WASD acceleration (units/s^2)
static constexpr f32 FREE_CAM_MAX_SPEED      = 1600.0f;  // WASD max world-space speed
static constexpr f32 FREE_CAM_FRICTION       = 6.0f;     // 1/s, velocity decay when no input
...
RtsControls::RtsControls()
    ...
    , m_camera_transition_from(Vec2{0.0f, 0.0f})
    , m_free_camera_vel(Vec2{0.0f, 0.0f})
    , m_free_camera_drag_target(Vec2{0.0f, 0.0f})
    , m_free_camera_drag_start_mouse(Vec2{0.0f, 0.0f})
    , m_free_camera_drag_start_pos(Vec2{0.0f, 0.0f})
...
void RtsControls::draw_rect_from_screen_box(const RtsSelectionBox& box, f32 thickness, bs_color color, u32 layer) {
    if (!m_state) return;
    Vec2 p0 = camera2d_screen_to_world(&m_state->camera, m_state->fb_width, m_state->fb_height, Vec2{ box.start_x, box.start_y });
    Vec2 p1 = camera2d_screen_to_world(&m_state->camera, m_state->fb_width, m_state->fb_height, Vec2{ box.end_x, box.end_y });
...
// =====================================================================================
void RtsControls::draw_selection_rect(const Ship* ship, f32 thickness, bs_color color, u32 layer) {
    if (!ship) return;
    f32 radius = ship_bounding_radius(ship);
    if (radius <= 0.0f) return;
    renderer_draw_rect_outline(ship->origin, Vec2{ radius * 2.0f, radius * 2.0f }, thickness, color, layer);
}
// =====================================================================================
void RtsControls::clear_fleet_orders() {
    if (!m_state) return;
    for (i32 i = 0; i < m_state->fleet.count(); ++i) {
        m_state->fleet.at(i).order = FleetOrder::None;
        m_state->fleet.at(i).move_target = Vec2{0.0f, 0.0f};
        m_state->fleet.at(i).attack_target = nullptr;
    }
}
// =====================================================================================
static b8 is_point_over_ship(Vec2 world_pos, const Ship* ship) {
    if (!ship) return FALSE;
    f32 dist = vec2_length(vec2_sub(world_pos, ship->origin));
    return dist <= ship_bounding_radius(ship);
}
// =====================================================================================
void RtsControls::update(f32 dt) {
    if (!m_state) return;
    if (m_state->mode != MODE_GLOBAL) {
        // Reset transient RTS state when leaving global mode.
        m_hovered_ship_idx = -1;
        m_hovered_enemy_idx = -1;
        m_box.active = FALSE;
        m_camera_transitioning = FALSE;
    ...
    if (m_piloted_idx < 0 || m_piloted_idx >= m_state->fleet.count())
        m_piloted_idx = 0;
    i32 mx, my;
    input_get_mouse_position(&mx, &my);
    Vec2 mouse_pos = camera2d_screen_to_world(&m_state->camera, m_state->fb_width, m_state->fb_height,
                                              Vec2{ (f32)mx, (f32)my });
    // ---- Hover detection over any fleet ship ------------------------------------------
    m_hovered_ship_idx = -1;
    for (i32 i = 0; i < m_state->fleet.count(); ++i) {
        if (is_point_over_ship(mouse_pos, &m_state->fleet.at(i).ship)) {
    ...
    for (i32 i = 0; i < m_state->combat_entity_count; ++i) {
        ...
        if (is_point_over_ship(mouse_pos, ce->ship)) {
    ...
    // ---- Free camera movement, selection, and order input (only while detached) ---------
    if (m_state->free_camera_active && !m_camera_transitioning) {
        ...
        if (input_dir.x != 0.0f || input_dir.y != 0.0f) {
            m_free_camera_vel = vec2_add(m_free_camera_vel,
                                         vec2_scale(input_dir, FREE_CAM_ACCEL * dt));
            f32 speed = vec2_length(m_free_camera_vel);
            if (speed > FREE_CAM_MAX_SPEED) {
                m_free_camera_vel = vec2_scale(m_free_camera_vel, FREE_CAM_MAX_SPEED / speed);
            }
        } else {
            f32 decay = expf(-FREE_CAM_FRICTION * dt);
            m_free_camera_vel = vec2_scale(m_free_camera_vel, decay);
        }
        // Middle-mouse drag.
        ...
        if (middle_down && !bs_imgui_wants_mouse()) {
            if (!middle_was_down) {
                ...
                m_free_camera_drag_start_pos = m_state->free_camera_pos;
                m_free_camera_drag_start_mouse = Vec2{ (f32)mx, (f32)my };
                m_free_camera_drag_target = m_state->free_camera_pos;
                m_free_camera_vel = Vec2{ 0.0f, 0.0f };
            } else {
                Vec2 screen_delta = Vec2{ (f32)mx - m_free_camera_drag_start_mouse.x,
                                          (f32)my - m_free_camera_drag_start_mouse.y };
                Vec2 world_delta = Vec2{ screen_delta.x / m_state->camera.zoom,
                                         -screen_delta.y / m_state->camera.zoom };
                m_free_camera_drag_target = vec2_sub(m_free_camera_drag_start_pos, world_delta);
            }
        }
        // Spring toward drag target.
        if (m_free_camera_dragging) {
            Vec2 error = vec2_sub(m_free_camera_drag_target, m_state->free_camera_pos);
            m_free_camera_vel = vec2_add(m_free_camera_vel,
                                         vec2_scale(error, FREE_CAM_DRAG_RESPONSIVE * dt));
            m_free_camera_vel = vec2_sub(m_free_camera_vel,
                                         vec2_scale(m_free_camera_vel, FREE_CAM_DRAG_DAMPING * dt));
        }
        // Apply velocity.
        m_state->free_camera_pos = vec2_add(m_state->free_camera_pos,
                                            vec2_scale(m_free_camera_vel, dt));
        m_state->camera.position = m_state->free_camera_pos;
        // ---- Box selection input -------------------------------------------------------
        ...
        if (m_box.active && left_released) {
            ...
            if (dx * dx + dy * dy < RTS_CLICK_THRESHOLD * RTS_CLICK_THRESHOLD) {
                ...
                m_state->fleet.clear_selection();
                if (m_hovered_ship_idx >= 0) {
                    m_state->fleet.set_selected(m_hovered_ship_idx, TRUE);
                }
            } else {
                // Drag: box-select all fleet ships whose origins fall inside the world box.
                Vec2 p0 = camera2d_screen_to_world(&m_state->camera, m_state->fb_width, m_state->fb_height,
                                                   Vec2{ m_box.start_x, m_box.start_y });
                Vec2 p1 = camera2d_screen_to_world(&m_state->camera, m_state->fb_width, m_state->fb_height,
                                                   Vec2{ m_box.end_x, m_box.end_y });
                m_state->fleet.clear_selection();
                for (i32 i = 0; i < m_state->fleet.count(); ++i) {
                    if (ship_inside_world_box(&m_state->fleet.at(i).ship, p0, p1)) {
                        m_state->fleet.set_selected(i, TRUE);
                    }
                }
            }
            m_box.active = FALSE;
    ...
    if (m_camera_transitioning) {
        ...
        Vec2 target = piloted->ship.origin;
        Vec2 new_pos = vec2_add(m_camera_transition_from,
                                vec2_scale(vec2_sub(target, m_camera_transition_from), eased));
        m_state->camera.position = new_pos;
        m_state->free_camera_pos = new_pos; // keep free_camera_pos in sync
        if (t >= 1.0f) {
            m_camera_transitioning = FALSE;
            m_state->free_camera_active = FALSE;
        }
    }
}
// =====================================================================================
...
void RtsControls::draw() {
    if (!m_state) return;
    b8 in_free_camera = (m_state->mode == MODE_GLOBAL && m_state->free_camera_active);
    ...
    if (in_free_camera) {
        ...
        for (i32 i = 0; i < m_state->fleet.count(); ++i) {
            if (m_state->fleet.is_selected(i)) {
                draw_selection_rect(&m_state->fleet.at(i).ship, BOX_SELECT_THICKNESS,
                                    SELECTION_RECT_COLOR, HOVER_CIRCLE_LAYER);
            }
        }
        // Draw move/attack markers for active orders.
        for (i32 i = 0; i < m_state->fleet.count(); ++i) {
            FleetShip& fs = m_state->fleet.at(i);
            if (fs.order == FleetOrder::Move) {
                draw_move_marker(fs.move_target, RTS_MOVE_MARKER_THICKNESS,
                                 RTS_MOVE_MARKER_COLOR, HOVER_CIRCLE_LAYER);
            } else if (fs.order == FleetOrder::Attack && fs.attack_target) {
                draw_attack_marker(fs.attack_target, BOX_SELECT_THICKNESS,
                                   bs_color{ 1.0f, 0.25f, 0.25f, 1.0f }, HOVER_CIRCLE_LAYER);
            }
        }
        // Hover ring around the hovered ship (fleet or not).
        if (m_hovered_ship_idx >= 0) {
            const Ship* ship = &m_state->fleet.at(m_hovered_ship_idx).ship;
            f32 radius = ship_bounding_radius(ship);
            if (radius > 0.0f) {
                bs_color col = HOVER_CIRCLE_COLOR;
                f32 pulse = 0.5f + 0.5f * sinf(m_state->elapsed_time * 3.0f);
                col.a *= 0.7f + 0.3f * pulse;
                draw_dashed_circle(ship->origin, radius, (u32)HOVER_DASH_SEGMENTS,
        ...
        if (bs_ui_begin_hud_panel("FLEET SHIP", BS_UI_ANCHOR_TOP_RIGHT, 190.0f)) {
            ...
            if (in_free_camera) {
                if (bs_ui_button("Pilot unit", !m_camera_transitioning)) {
                    m_state->fleet.set_piloted(m_state->fleet.any_selected()
                                               ? m_state->fleet.first_selected()
                                               : 0);
                    m_piloted_idx = m_state->fleet.piloted_index();
                    m_camera_transitioning = TRUE;
                    m_camera_transition_t = 0.0f;
                    m_camera_transition_from = m_state->camera.position;
                    m_free_camera_vel = Vec2{ 0.0f, 0.0f };
                    m_free_camera_dragging = FALSE;
                ...
                if (bs_ui_button("Auto-pilot / RTS", !m_camera_transitioning)) {
                    m_state->free_camera_active = TRUE;
                    m_state->free_camera_pos = m_state->camera.position;
                    m_camera_transitioning = FALSE;
...
// =====================================================================================
void RtsControls::draw_attack_marker(const Ship* ship, f32 thickness, bs_color color, u32 layer) {
    if (!ship) return;
    f32 radius = ship_bounding_radius(ship);
    if (radius <= 0.0f) return;
