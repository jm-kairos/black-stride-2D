...
// Repulsion from a too-close neighbor: zero at min_sep and beyond, rising to max_speed at
// contact. `to_other` points AT the neighbor (dist = |to_other|), the result points away.
bs_math::Vec2 separation(bs_math::Vec2 to_other, f32 dist, f32 min_sep, f32 max_speed);
...
// Same control law as apply_face but WITHOUT pose integration, for ships whose integrator runs
// elsewhere (FleetShip::simulate integrates every fleet member every frame -- apply_face there
// would integrate the pose twice per frame and the ship flies at double speed).
void control_face(Ship* sh, ShipFlight* fl, bs_math::Vec2 desired_vel, bs_math::Vec2 face_dir,
                  f32 accel, f32 max_speed, f32 turn_rate, f32 dt);
