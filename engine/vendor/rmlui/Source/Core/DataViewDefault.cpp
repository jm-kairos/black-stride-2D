...
//  'data-for' must run before same-depth views that read its array (e.g. a 'data-class' on the
//  same element), so it can add/remove children before those views resolve now-stale indices.
//  Without this, shrinking an array logs spurious "Data array index out of bounds" warnings.
