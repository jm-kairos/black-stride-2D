// starfield_layer.frag.hlsl — procedural starfield layer (one draw per parallax layer)
...
//   params2 : seed, 0, 0, 0
//   params3 : star_pos_x, star_pos_y, dazzle_inner_radius, dazzle_outer_radius
// Uniform layout (b1, space3):
//   params4 : dazzle_intensity, 0, 0, 0
...
cbuffer StarfieldLayerParams : register(b0, space3)
{
    ...
    float4 params3; // star_pos_x, star_pos_y, dazzle_inner, dazzle_outer
};

cbuffer StarfieldDazzleParams : register(b1, space3)
{
    float4 params4; // dazzle_intensity, 0, 0, 0
};
...
float4 main(PSInput input) : SV_Target0
{
    ...
    // Star dazzle: suppress faint stars near the bright central star.
    float2 star_pos = params3.xy;
    ...
    if (dazzle_outer > dazzle_inner && dazzle_intensity > 0.0) {
        float d = distance(world_pos, star_pos);
